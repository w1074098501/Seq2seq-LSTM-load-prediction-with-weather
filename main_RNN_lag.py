...
@tf.function
def train_rnn_step(input_load,input_w,targets):
    loss = 0
    # inputs shape [batch_size,input_shape]
    with tf.GradientTape() as tape:
        outputs = rnn_model(input_load,input_w)
...
def evaluate_rnn(input_load,input_w,targets,training):
...
train_dataset = tf.data.Dataset.from_tensor_slices((load_norm_prev[:train_size,:,:],weather_feat_curr[:train_size,:,:],load_norm_curr[:train_size,:,:]))
test_dataset = tf.data.Dataset.from_tensor_slices((load_norm_prev[train_size:,:,:],weather_feat_curr[train_size:,:,:],load_norm_curr[train_size:,:,:]))
...
result_path = 'WL_prevStateNew_1LSTM%d_MLP_%dntime%d_Adam_lr%.5f'%(hidden_units,nprev,ntime,learning_rate)
...
with open(os.path.join(result_path,Training_log_file),'w') as filehandle:
    ...
    filehandle.write('Use weather features as inputs (together with initial states) to predict curr load\n')
...
for epoch in range(1,600+1):
    ...
    total_loss = 0
    total_mse = 0
    for (batch, (inp_load,inp_wth,targ)) in enumerate(train_dataset.take(steps_per_epoch)):
        batch_mse, batch_loss,grad_norm = train_rnn_step(inp_load,inp_wth,targ)
    ...
    ep_rmse, ep_mape, ep_mae = evaluate_rnn(inp_load,inp_wth,targ,training=True)            


    for (batch, (inp_load,inp_wth,targ)) in enumerate(test_dataset.take(1)):
        curr_test_rmse, curr_test_mape, curr_test_mae = evaluate_rnn(inp_load,inp_wth,targ,training=False)
...
'''result_path = "WL_prevStateNew_1LSTM80_MLP_144ntime72_Adam_lr0.00200"
epoch=600
rnn_model = build_rnn()
zx = tf.zeros([50,ntime,1])
zw = tf.ones([50,ntime,6])
_ = rnn_model(zx,zw)
...
weat = weather_feat_curr[train_size:,:,:]
targets = load_norm_curr[train_size:,:,:]*(load_max-load_min)+ load_min

outputs = rnn_model(inputs,weat)*(load_max-load_min)+ load_min
